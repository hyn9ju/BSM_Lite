...
/* "이 유형과 무엇이 다른가요" 문장 속에 등장하는 다른 스타일 이름을 굵게 강조한다 */
var STYLE_NAMES=Object.values(S).map(function(v){return v.name;}).sort(function(a,b){return b.length-a.length;});
function boldStyleNames(t){
  var out=esc(t);
  STYLE_NAMES.forEach(function(n){
    out=out.split(esc(n)).join('<b>'+esc(n)+'</b>');
  });
  return out;
}
...
function view(id){
  ...
  window.scrollTo(0,0);
  document.documentElement.style.setProperty('--accent',id==='viewResult'?curAccent:'#6C5CE0');
  document.documentElement.style.setProperty('--accent-ink',id==='viewResult'?curAccentInk:'#FFFFFF');
  document.documentElement.style.setProperty('--accent-text',id==='viewResult'?curAccentText:'#6C5CE0');
}
var curAccent='#6C5CE0', curAccentInk='#FFFFFF', curAccentText='#6C5CE0';
...
function showResult(code,opt){
  ...
  curAccent=s.accent; curAccentInk=readableOn(s.accent); curAccentText=textSafeOnWhite(s.accent);
  view('viewResult');
  var card=s.card;
  ...
  host.innerHTML=
   ...
   +(s.diff?'<div class="rsec"><h3>이 유형과 무엇이 다른가요</h3><p class="diff">'+boldStyleNames(s.diff)+'</p></div>':'')
...
/* ===== 기타 ===== */
/* v0.2.0: 밝은 테마로 바뀌면서 유형별 강조색을 흰 카드 위에 '글자색'으로 쓰는 곳이
   생겼다. 이 색들은 원래 어두운 배경 위에서 또렷하도록 고른 색이라 흰 배경에서는
   대비가 부족한 경우가 많다 — 흰 바탕 대비 4.5:1을 만족할 때까지 밝기만 낮춰
   글자로 써도 안전한 색을 만든다. 테두리·배지 등 원색 그대로 쓰는 곳은 그대로 둔다. */
function hex2rgb(hex){
  var h=String(hex).replace('#',''); if(h.length===3)h=h[0]+h[0]+h[1]+h[1]+h[2]+h[2];
  var n=parseInt(h,16); return [n>>16,(n>>8)&255,n&255];
}
function relLum(hex){
  var rgb=hex2rgb(hex).map(function(v){v/=255;return v<=0.03928?v/12.92:Math.pow((v+0.055)/1.055,2.4);});
...
function textSafeOnWhite(hex){
  if(contrastRatio(hex,'#FFFFFF')>=4.5)return hex;
  var rgb=hex2rgb(hex);
  for(var i=0;i<40;i++){
    rgb=rgb.map(function(v){return v*0.92;});
    var c=rgb2hex(rgb);
